...
function draw() {
  ...
  for (let i = 0; i < playerArrows.length; i++) {
    if (playerArrows[i] !== undefined) {
      ...
      let board1Collision = Matter.SAT.collides(
        board1.body,
        playerArrows[i].body
      );

      let board2Collision = Matter.SAT.collides(
        board2.body,
        playerArrows[i].body
      );
      ...
      let posX = playerArrows[i].body.position.x;
      let posY = playerArrows[i].body.position.y;
